...
gsc <- GeneSetCollection(c(c2BroadSets, de_fc_dysplasiaGS))
gsets <- geneIds(gsc)
```
...
histology_factors <- factor(se.filt$histology)
histology_integers <- as.integer(histology_factors)

levels(histology_factors)
histology_integers
```
...
gseapermpheno <- fgseaLabel(gsets, assays(se.filt)$logCPM, histology_integers,10000, minSize=5, maxSize=300)
head(gseapermpheno[order(gseapermpheno$pval), ])
...
``` {r}
stats <- tt_dysplasia$t
names(stats) <- rownames(tt_dysplasia)
gseapermgsets <- fgsea(gsets, stats, minSize=5, maxSize=300)
head(gseapermgsets[order(gseapermgsets$pval), ])
gseapermpheno[gseapermpheno$padj < 0.01, ]
gseapermgsets[gseapermgsets$padj < 0.01, ]
```
...
```{r}
par(mfrow=c(1, 2))
hist(gseapermpheno$pval, xlab="P-value", main="Permuting phenotypes", las=1)
hist(gseapermgsets$pval, xlab="P-value", main="Permuting gene sets", las=1)
plotEnrichment(gsets$DE_FC_DYS, stats)
...
```{r}
DEpwys <- gseapermgsets$pathway[gseapermgsets$padj < 0.01]
plotGseaTable(gsets[DEpwys], stats, gseapermgsets, gseaParam=0.5)
...
```{r} 
library(limma)
gsetidx <- ids2indices(geneIds(gsc), rownames(se.filt))
camerares <- camera(y=assays(se.filt)$logCPM, index=gsetidx, design=mod, contrast=2)
head(camerares, 5)
```
...
```{r}
hist(camerares$PValue, xlab="P-value", main="Camera", las=1)
```
...
gsc <- GeneSetCollection(c(c2BroadSets, de_fc_dysplasiaGS, de_fc_smokerGS))
gsets <- geneIds(gsc)
```
...
hist(gseapermpheno$pval, xlab="P-value", main="Permuting phenotypes", las=1)
hist(gseapermgsets$pval, xlab="P-value", main="Permuting gene sets", las=1)
plotEnrichment(gsets$DE_FC_SMOKER, stats)
