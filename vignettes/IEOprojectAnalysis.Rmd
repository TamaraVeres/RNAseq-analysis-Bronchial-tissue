...
```{r}
# Calculate average log2 CPM values
avgexp <- rowMeans(assays(se)$logCPM)

# Filter to keep only values between 5 and 10
avgexp_filtered <- avgexp[avgexp >= 5 & avgexp <= 10]

# Create histogram with the filtered data
hist(avgexp_filtered, xlab="log2 CPM", main="", las=1, 
     breaks=40, # Adjust the number of breaks to improve histogram resolution
     xlim=c(5, 10)) # Set x-axis limits to show only the filtered range
...
se.filt.nona <- se.filt[!grepl("dysplasia status: NA", se.filt$characteristics_ch1.7), ]
dge.filt.nona <- dge.filt[!grepl("dysplasia status: NA", se.filt$characteristics_ch1.7), ]
logCPM <- cpm(dge.filt.nona, log=TRUE, prior.count=3)
d <- as.dist(1-cor(logCPM, method="spearman"))
sampleClustering <- hclust(d)
batch <- as.integer(se.filt.nona$histology)
sampleDendrogram <- as.dendrogram(sampleClustering, hang=0.1)
names(batch) <- colnames(se.filt.nona)
outcome <- colnames(se.filt.nona)
names(outcome) <- colnames(se.filt.nona)
...
plot(sampleDendrogram, main="Hierarchical clustering of samples",
     cex=0.7)
legend("topright", levels(se.filt.nona$pack_years),
       fill=seq_len(nlevels(se.filt.nona$pack_years)))
...
legend("bottomleft", levels(se.filt$pack_years),
       fill=seq_len(nlevels(se.filt$pack_years)), inset=0.05)
```
