...
```{r}
log2fc_copd <- copd - no_copd
ranking <- order(abs(log2fc_copd), decreasing=TRUE)
fold_change_copd <- data.frame(EntrezID=names(log2fc_copd[ranking]),
                                    Log2FC=round(log2fc_copd[ranking], digits=3),
                                    FC=round(2^log2fc_copd[ranking], digits=3),
                                    "1/FC"=round(2^(-log2fc_copd[ranking]), digits=3),
                                    row.names=rowData(se.filt)$symbol[ranking],
                                    check.names=FALSE)
de_fc_copd <- head(fold_change_copd, n=10)
de_fc_copd
```
...
-   If a gene has a **positive** Log2FC value, it means said gene is
    **FC** times more upregulated in samples with *COPD* compared to in
    samples *without COPD*.

    -   For example, TPRXL is more upregulated in samples with COPD than
        in samples without COPD

-   If a gene has a **negative** Log2FC value, it means said gene is
    **1/FC** times more upregulated in samples *without COPD* compared
    to in samples with *COPD*

    -   For example, SLC22A16 is more upregulated in samples with COPD
        than in samples without COPD
