...
tmpdf <- data.frame(Identifer=colnames(se),
                    ...
                    check.names=FALSE)
short_tmpdf <- head(tmpdf, 40)
ktab <- kable(short_tmpdf, caption="Phenotypical variables (first 40 rows).")
ktab <- kable_styling(ktab, position="center")
save_kable(ktab, file="../doc/phenotypical_variables.html", self_contained=TRUE)
```

To see the full table for all samples click <a href="../doc/phenotypical_variables.html" target="_blank">link</a>.
...
### Question to Answer Formulation
...
All of the samples in our dataset have either been or currently are smokers. Based on the results we’ve obtained on this first part of our project, we’ve decided to use this variable to design our new groups. We obtain: Current Smokers (n = r length(se.filtsmoker[se.filt
smoker == “Current smoker”])) and Former smokers (n = r length(se.filtsmoker[se.filt
...
These results will help us gain an insight into How the genome changes once a person stops smoking.

From here on out, every step will be done once for the Dysplasia grouping (samples with PML vs without) to reproduce the paper’s findings, and once for the Smoking status grouping, to obtain our new results.
...
We create a custom gene set (de_fc_smokerGS) using the Entrez IDs of differentially expressed genes in smoking status. We then combine this custom gene set with the previously filtered gene sets into a single Gene Set Collection (gsc).
...
This pathway involves the hydrolysis of GTP and the subsequent joining of the 60S ribosomal subunit, which is essential for the formation of functional ribosomes and the initiation of protein synthesis. Smoking can interfere with this process by inducing oxidative stress and causing damage to ribosomal components. Such disruptions in ribosome assembly and function can lead to abnormal protein synthesis, which is a common feature in rapidly dividing cancer cells and precancerous conditions like dysplasia. Proper protein synthesis is crucial for maintaining normal cell growth and differentiation, and any impairment in this pathway can contribute to the cellular transformations seen in dysplasia and cancer.


### Current and former smoker results

REACTOME_INSULIN_SYNTHESIS_AND_SECRETION:
The insulin synthesis and secretion pathway is crucial for regulating blood glucose levels and involves various cellular processes in the pancreas.
In current smokers, chronic nicotine exposure can impair insulin production and secretion due to oxidative stress and inflammation, potentially leading to insulin resistance and increased risk of diabetes.
For former smokers, quitting smoking may improve insulin sensitivity and normalize insulin secretion dynamics, reducing the risk of diabetes.

KEGG_OXIDATIVE_PHOSPHORYLATION:
This pathway is essential for ATP production through the mitochondrial electron transport chain.
In current smokers, smoking-induced oxidative stress can impair mitochondrial function and oxidative phosphorylation, leading to decreased cellular energy production and increased oxidative damage.
For former smokers, cessation may lead to recovery of mitochondrial function and improved oxidative phosphorylation efficiency, enhancing overall cellular energy metabolism.

REACTOME_TRANSLATION:
The translation pathway is vital for protein synthesis, involving ribosomal assembly and function.
In current smokers, smoking-related oxidative stress and inflammation can disrupt ribosomal function and protein synthesis, contributing to abnormal cell growth and protein misfolding.
Former smokers may experience a normalization of translation processes, improving protein synthesis and reducing the risk of cellular dysfunction.

REACTOME_GTP_HYDROLYSIS_AND_JOINING_OF_THE_60S_RIBOSOMAL_SUBUNIT:
This pathway involves the hydrolysis of GTP and the joining of the 60S ribosomal subunit, crucial for forming functional ribosomes.
In current smokers, oxidative stress from smoking can interfere with ribosome assembly, impairing protein synthesis and contributing to cellular transformation and cancer risk.
For former smokers, quitting smoking may lead to the recovery of ribosomal function and proper protein synthesis, reducing the likelihood of cellular abnormalities.

REACTOME_GLUCOSE_REGULATION_OF_INSULIN_SECRETION:
This pathway is involved in the regulation of insulin secretion in response to glucose levels.
In current smokers, smoking can disrupt glucose regulation and insulin secretion through oxidative stress and inflammatory mechanisms, increasing the risk of metabolic disorders like diabetes.
Former smokers might see improvements in glucose regulation and insulin secretion dynamics after quitting, lowering their risk of metabolic diseases.

# Discussion

We ended up following two different pipelines and therefore reached two sets of results: one aims at reproducing the results found in the original @Beane17, and the other follows what has become our new question to be answered by analyzing this RNA-seq dataset: "How does the genome change between people that currently smoked versus people that have quit smoking?"
